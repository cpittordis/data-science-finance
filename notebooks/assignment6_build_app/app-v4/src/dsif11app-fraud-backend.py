...
@app.post("/predict_df")
def predict_df(df_input: DataFrameInput):

    #print("helloooo  ")
    # Convert input to DataFrame
    df = pd.DataFrame(df_input.data)
    
    # Preprocess data (modify according to your model's needs)
    # Example: df = preprocess(df)
    
    # Make predictions
    predictions = loaded_pipeline.predict(df)
    predictions_proba_0 = loaded_pipeline.predict_proba(df)[:,0]
    predictions_proba_1 = loaded_pipeline.predict_proba(df)[:,1]
    
    # Return predictions as pandas Series
    return {
        "predictions": pd.Series(predictions).to_list(),
        "predictions_proba_0" : pd.Series(predictions_proba_0).to_list(),
        "predictions_proba_1" : pd.Series(predictions_proba_1).to_list()
        }
